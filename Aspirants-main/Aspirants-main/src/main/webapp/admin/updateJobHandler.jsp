...
<%
    ...
    try {
        ...
        String sql = "UPDATE jobs SET job_title=?, description=?, location=?, salary=?, eligibility=?, last_date=?, has_10th=?, has_12th=?, has_diploma=?, has_graduation=?, has_post_graduation=?, age_limit=?, link=?, category=? WHERE job_id=?";
        ...
        pstmt.setInt(15, jobId);

        int result = pstmt.executeUpdate();
        if (result > 0) {
            response.sendRedirect("manageJobs.jsp?success=Job updated successfully");
        } else {
            response.sendRedirect("editJob.jsp?job_id=" + jobId + "&error=Update failed");
        }
    } catch (Exception e) {
        e.printStackTrace();
        response.sendRedirect("editJob.jsp?job_id=" + jobId + "&error=Something went wrong");
    } finally {
